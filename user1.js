...
socket.on('connect', () => {
    console.log('User 1 connected to server:', socket.id);

    // Function to send a message at random intervals
    const sendMessage = () => {
        const message = `Hello from User 1 at ${new Date().toLocaleTimeString()}`;
...
socket.on('message', (data) => {
    console.log('User 1 received message from server:', data);
});
